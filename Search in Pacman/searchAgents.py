...
def cornersHeuristic(state, problem):
    ...
    while corners != []:
        dist = []
        for x_2, y_2 in corners:
            dist += [abs(x_1-x_2)+abs(y_1-y_2)]
        h += min(dist)
        x_1, y_1 = corners.pop(dist.index(min(dist)))
...
def foodHeuristic(state, problem):
    ...
    dist = []
    if foodList != []:
        for x_2, y_2 in foodList:
            #print(x_2,y_2)
            #dist += [abs(x_1-x_2)+abs(y_1-y_2)]
            #dist += [abs(x_1-x_2)+abs(y_1-y_2)+max([abs(x_2-x_3)+abs(y_2-y_3) for x_3, y_3 in foodList])]
            dist += [mazeDistance((x_1,y_1), (x_2,y_2), problem.startingGameState)]
        h = max(dist)
    return h
